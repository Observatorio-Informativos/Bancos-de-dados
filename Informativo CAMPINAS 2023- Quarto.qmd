...
setwd("C:/Users/User/Documents/Observatório PUC Camp/Informativos/Informativos Campinas")
```
...
**Volume 1 \| N. `r mes_nom` \| `r format(Sys.time(), '%Y')`**
...
tb12 <- sp_imp %>% 
  ...
  filter(CO_PAIS == 249 | CO_PAIS == 160 | CO_PAIS == 858 | CO_PAIS == 676 | CO_PAIS == 190 | 
           CO_PAIS == 23 | CO_PAIS == 399 | CO_PAIS == 493 | CO_PAIS == 361 | CO_PAIS == 245) %>% 
...
As análises mais recentes do Observatório PUC-Campinas apontam para um ano de (...) nas importações (...%) e (...) das exportações (...%). Em relação as últimas previsões, nota-se que os dados de exportação do mês 04/2023 indicaram uma (...) na taxa de variação das exportações. A redução das importações pode estar relacionada a queda no valor importad

```{r}

```

o tanto de bens acabados como bens intermediários. Reduções expressivas na importação de bens intermediários indicam, em geral, queda do ritmo da produção industrial.
